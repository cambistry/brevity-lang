...
```
(This generalizes Phase 3 of the Layer A note: the scanner matches `#<…>` anywhere in string fields, not only whole-string payload values. Reserves `#<…>` as wire-level syntax in strings; escape convention implemented 2026-04-23.)
...
The naive path — `realDiv.innerHTML = inner_html_string` — does not work when the string contains `#<…>` tokens. HTML's tokenizer sees `#` as text, then `<pub` as a tag-open, so `#<pub @0>` gets mangled into text `#` + element `<pub @0>`. Tokens do not survive the browser's HTML parser.
...
One representation (`#<…>`) throughout — wire, string, runtime. Escape rule: `\>` inside, `\#<` outside.
...
5. ~~Escape convention for literal `#<…>` in user-supplied strings.~~ **Implemented 2026-04-23** (`\#<` outside, `\>` inside addresses).
