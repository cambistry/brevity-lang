import { expectBehavior, compileSource } from '../helpers.js';

// ═══════════════════════════════════════════════════════════════════════════════
// Declarations and typed RHS
// ═══════════════════════════════════════════════════════════════════════════════

describe('type declarations + typed RHS', () => {
  const script = `
      @declThenUse
        =
        x Integer
        x = 1 as Integer
        -> result: x

      @typedInt = { x = 1 as Integer; -> result: x }
      @typedText = { x = "hello" as Text; -> result: x }

      @typedExpr
        =
        a: Integer
        b: Integer
        =
        x = (a + b) as Integer
        -> result: x

      @bothSides = { x Integer = 2 as Integer; -> result: x }
      @hoisting = {
        x = 1 as Integer
        x Integer
        -> result: x
      }
      @tripleDecl = {
        x Integer
        x Integer = 5 as Integer
        -> result: x
      }
  `;

  it('x : Integer before assignment — decl then use', async () => {
    await expectBehavior(script, {
      input: { id: '1', op: '@declThenUse', from: 'c' },
      output: { id: '1', 'bv-a': { result: 'Integer' }, re: { result: 1 }, to: 'c' },
    });
  });

  it('x = 1 as Integer — typed RHS', async () => {
    await expectBehavior(script, {
      input: { id: '2', op: '@typedInt', from: 'c' },
      output: { id: '2', 'bv-a': { result: 'Integer' }, re: { result: 1 }, to: 'c' },
    });
  });

  it('x = "hello" as Text — typed RHS string', async () => {
    await expectBehavior(script, {
      input: { id: '3', op: '@typedText', from: 'c' },
      output: { id: '3', 'bv-a': { result: 'Text' }, re: { result: 'hello' }, to: 'c' },
    });
  });

  it('x = (a + b) as Integer — typed RHS expression', async () => {
    await expectBehavior(script, {
      input: { id: '4', op: [{ a: 3, b: 4 }, '@typedExpr'], 'bv-a': [{ a: 'Integer', b: 'Integer' }], from: 'c' },
      output: { id: '4', 'bv-a': { result: 'Integer' }, re: { result: 7 }, to: 'c' },
    });
  });

  it('x Integer = 2 as Integer — type on both sides', async () => {
    await expectBehavior(script, {
      input: { id: '5', op: '@bothSides', from: 'c' },
      output: { id: '5', 'bv-a': { result: 'Integer' }, re: { result: 2 }, to: 'c' },
    });
  });

  it('x = 1 as Integer then x : Integer — hoisting', async () => {
    await expectBehavior(script, {
      input: { id: '6', op: '@hoisting', from: 'c' },
      output: { id: '6', 'bv-a': { result: 'Integer' }, re: { result: 1 }, to: 'c' },
    });
  });

  it('x : Integer declared three times — all legal', async () => {
    await expectBehavior(script, {
      input: { id: '7', op: '@tripleDecl', from: 'c' },
      output: { id: '7', 'bv-a': { result: 'Integer' }, re: { result: 5 }, to: 'c' },
    });
  });
});

// ═══════════════════════════════════════════════════════════════════════════════
// Compile checks
// ═══════════════════════════════════════════════════════════════════════════════

describe('type declarations — compile checks', () => {
  it('x : Integer — bare decl compiles without error', () => {
    expect(() => compileSource(`@go = { x Integer; -> result: 0 as Integer }\n`)).not.toThrow();
  });
});

describe('conflicting type declarations — compile errors', () => {
  it('bare decl then conflicting TypedAssign → error', () => {
    expect(() => compileSource(`@go = { x Integer; x Text = "hello"; -> result: x }\n`))
      .toThrow(/Conflicting type declarations for 'x'/);
  });

  it('TypedAssign then conflicting bare decl', () => {
    expect(() => compileSource(`@go = { x Text = "hello"; x Integer; -> result: x }\n`))
      .toThrow(/Conflicting type declarations for 'x'/);
  });

  it('x Integer = "hello" as Text — same-line conflict', () => {
    expect(() => compileSource(`@go = { x Integer = "hello" as Text; -> result: x }\n`))
      .toThrow(/Conflicting type declarations for 'x'/);
  });

  it('two conflicting bare decls', () => {
    expect(() => compileSource(`@go = { x Integer; x Text; -> result: x }\n`))
      .toThrow(/Conflicting type declarations for 'x'/);
  });
});
