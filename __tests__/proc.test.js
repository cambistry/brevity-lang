import compile from '../index.js';
import { expectReply } from './helpers.js';

describe('proc — basic call from handler', () => {
  it('handler calls proc, destructures result', async () => {
    const source = `
      on foo()
        result: x : Integer = square(10)
        reply :x

      proc square(num : Integer)
        sq : Integer = num * num
        reply(result: sq : Integer)
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'foo', from: 'caller' },
      reply: { id: '1', 'bv-a': [{ x: 'Integer' }, 'foo'], re: [{ x: 100 }, 'foo'], to: 'caller' },
    });
  });

  it('proc result assigned as whole Structure, then destructured', async () => {
    const source = `
      on foo()
        s : Structure = square(10)
        result: x : Integer = s
        reply :x

      proc square(num : Integer)
        sq : Integer = num * num
        reply(result: sq : Integer)
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'foo', from: 'caller' },
      reply: { id: '1', 'bv-a': [{ x: 'Integer' }, 'foo'], re: [{ x: 100 }, 'foo'], to: 'caller' },
    });
  });

  it('proc with named arg', async () => {
    const source = `
      on greet(:name : Text)
        result: msg : Text = format(name)
        reply :msg

      proc format(val : Text)
        reply(result: val : Text)
    `;
    await expectReply({
      source,
      receive: { id: '1', op: [{ name: 'world' }, 'greet'], 'bv-a': [{ name: 'Text' }, 'greet'], from: 'caller' },
      reply: { id: '1', 'bv-a': [{ msg: 'Text' }, 'greet'], re: [{ msg: 'world' }, 'greet'], to: 'caller' },
    });
  });

  it('proc called twice in same handler with different args', async () => {
    const source = `
      on foo()
        result: a : Integer = square(3)
        result: b : Integer = square(4)
        reply sum: a + b : Integer

      proc square(num : Integer)
        sq : Integer = num * num
        reply(result: sq : Integer)
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'foo', from: 'caller' },
      reply: { id: '1', 'bv-a': [{ sum: 'Integer' }, 'foo'], re: [{ sum: 25 }, 'foo'], to: 'caller' },
    });
  });
});

describe('proc — mixed destructure from proc result', () => {
  it('positional + named + key-mapped all bound correctly', async () => {
    const source = `
      on foo()
        a : Integer, b : Integer, :c : Text, d: x : Text = sub()
        reply pa: a + b : Integer, nc: c, nd: x

      proc sub

        reply
          10 : Integer
          20 : Integer
          c: "v1" : Text
          d: "v2" : Text
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'foo', from: 'caller' },
      reply: {
        id: '1', 'bv-a': [{ nc: 'Text', nd: 'Text', pa: 'Integer' }, 'foo'], re: [{ pa: 30, nc: 'v1', nd: 'v2' }, 'foo'], to: 'caller',
      },
    });
  });
});

describe('proc — namespace', () => {
  it('on and proc with the same name throws at compile time', () => {
    const source = `
      on square()
        reply result: 0 : Integer

      proc square(num : Integer)
        reply(result: num : Integer)
    `;
    expect(() => compile(source)).toThrow(/'square' is declared as both/);
  });
});

describe('proc — direct call harness', () => {
  it.todo('proc exposed as public method for direct testing');
});

describe('proc — plain assignment arity', () => {
  it('plain assign from proc returning 1 positional unwraps correctly', async () => {
    const source = `
      on test()
        a : Integer = getOne()
        reply result: a

      proc getOne

        reply 42 : Integer
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'test', from: 'caller' },
      reply: { id: '1', 'bv-a': [{ result: 'Integer' }, 'test'], re: [{ result: 42 }, 'test'], to: 'caller' },
    });
  });

  it('plain assign from proc returning 2 positionals throws at runtime', async () => {
    const source = `
      on test()
        a : Integer = getTwo()
        reply result: a

      proc getTwo

        reply(1 : Integer, 2 : Integer)
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'test', from: 'caller' },
      reply: { id: '1', ex: { test: 'error' }, to: 'caller' },
    });
  });
});

// ─── whitespace-only blank line ───────────────────────────────────────────────

describe('proc — spacious proc with whitespace-only blank line', () => {
  it('whitespace-only blank line terminates spacious proc param block', async () => {
    const source = `
      on go()
        doubled : Integer = double(5)
        reply :doubled

      proc double
        n : Integer
      ${'  '}
        reply n * 2 : Integer
    `;
    await expectReply({
      source,
      receive: { id: '1', op: 'go', from: 'caller' },
      reply: {
        id: '1', 'bv-a': [{ doubled: 'Integer' }, 'go'], re: [{ doubled: 10 }, 'go'], to: 'caller',
      },
    });
  });
});
