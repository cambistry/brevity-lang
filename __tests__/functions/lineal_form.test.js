...
describe('lineal function — param styles', () => {
  ...
  it('no-arg — single =', async () => {
    await expectBehavior({ script, receive: { id: '1', op: '@noArgSingle', from: 'c' }, reply: { id: '1', 'bv-a': { x: 'Integer' }, re: { x: 42 }, to: 'c' } });
  });

  it('no-arg — double =', async () => {
    await expectBehavior({ script, receive: { id: '2', op: '@noArgDouble', from: 'c' }, reply: { id: '2', 'bv-a': { x: 'Integer' }, re: { x: 42 }, to: 'c' } });
  });

  it('single positional param', async () => {
    await expectBehavior({ script, receive: { id: '3', op: '@singlePos', from: 'c' }, reply: { id: '3', 'bv-a': { x: 'Integer' }, re: { x: 10 }, to: 'c' } });
  });

  it('multiple positional params', async () => {
    await expectBehavior({ script, receive: { id: '4', op: '@multiPos', from: 'c' }, reply: { id: '4', 'bv-a': { s: 'Integer' }, re: { s: 7 }, to: 'c' } });
  });

  it('named param (sigil)', async () => {
    await expectBehavior({ script, receive: { id: '5', op: '@named', from: 'c' }, reply: { id: '5', 'bv-a': { msg: 'Text' }, re: { msg: 'world' }, to: 'c' } });
  });

  it('mixed positional + named', async () => {
    await expectBehavior({ script, receive: { id: '6', op: '@mixed', from: 'c' }, reply: { id: '6', 'bv-a': { x: 'Integer' }, re: { x: 10 }, to: 'c' } });
  });

  it('key-mapped param', async () => {
    await expectBehavior({ script, receive: { id: '7', op: '@keyed', from: 'c' }, reply: { id: '7', 'bv-a': { x: 'Text' }, re: { x: 'hello' }, to: 'c' } });
  });
});
...
describe('lineal function — body and return forms', () => {
  ...
  it('multi-statement body', async () => {
    await expectBehavior({ script, receive: { id: '1', op: '@multiStmt', from: 'c' }, reply: { id: '1', 'bv-a': { x: 'Integer' }, re: { x: 10 }, to: 'c' } });
  });

  it('lineal return (-> on own line, fields below)', async () => {
    await expectBehavior({ script, receive: { id: '2', op: '@spaciousReturn', from: 'c' }, reply: { id: '2', 'bv-a': { a: 'Integer', b: 'Text' }, re: { a: 10, b: 'hello' }, to: 'c' } });
  });
...
describe('lineal function — composition', () => {
  ...
  it('multiple lineal functions in same actor', async () => {
    await expectBehavior({ script, receive: { id: '1', op: '@multiFn', from: 'c' }, reply: { id: '1', 'bv-a': { sum: 'Integer' }, re: { sum: 25 }, to: 'c' } });
  });

  it('lineal function calls another lineal function', async () => {
    await expectBehavior({ script, receive: { id: '2', op: '@crossCall', from: 'c' }, reply: { id: '2', 'bv-a': { x: 'Integer' }, re: { x: 20 }, to: 'c' } });
  });

  it('lineal top-level + delimited lambda in public function', async () => {
    await expectBehavior({ script, receive: { id: '3', op: '@denseSpacious', from: 'c' }, reply: { id: '3', 'bv-a': { extra: 'Integer' }, re: { extra: 26 }, to: 'c' } });
  });
});
  ...
  it('side-effect-only function with dot', async () => {
    const script = `
      ...
      fire
        ...
        .
    `;
    await expectBehavior({ script, receive: { id: '1', op: '@go', from: 'c' }, reply: { id: '1', 'bv-a': { answer: 'Text' }, re: { answer: 'ok' }, to: 'c' } });
  });
});
