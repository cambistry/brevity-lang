...
describe('self-as clauses', () => {
  ...
  it('self as Integer — literal cast', async () => {
    await expectBehavior({ script, receive: { id: '1', op: '@asInt', from: 'c' }, reply: { id: '1', 'bv-a': ['Integer'], re: [1], to: 'c' } });
  });

  it('self as Text — literal cast', async () => {
    await expectBehavior({ script, receive: { id: '2', op: '@asText', from: 'c' }, reply: { id: '2', 'bv-a': ['Text'], re: ['one'], to: 'c' } });
  });

  it('self as Boolean — literal cast', async () => {
    await expectBehavior({ script, receive: { id: '3', op: '@asBool', from: 'c' }, reply: { id: '3', 'bv-a': ['Boolean'], re: [true], to: 'c' } });
  });
  ...
  it('untyped assignment — no cast, still works via ref', async () => {
    await expectBehavior({ script, receive: { id: '5', op: '@untypedRef', from: 'c' }, reply: { id: '5', 'bv-a': { answer: 'Text' }, re: { answer: 'world' }, to: 'c' } });
  });

  it('negated catch-all — as !Self (Integer target)', async () => {
    await expectBehavior({ script, receive: { id: '6', op: '@negatedInt', from: 'c' }, reply: { id: '6', 'bv-a': ['Integer'], re: [0], to: 'c' } });
  });

  it('negated catch-all — as !Self (Text target)', async () => {
    await expectBehavior({ script, receive: { id: '7', op: '@negatedText', from: 'c' }, reply: { id: '7', 'bv-a': ['Text'], re: ['default'], to: 'c' } });
  });

  it('self-as clause — two-line form', async () => {
    await expectBehavior({ script, receive: { id: '8', op: '@twoLineForm', from: 'c' }, reply: { id: '8', 'bv-a': ['Integer'], re: [1], to: 'c' } });
  });
