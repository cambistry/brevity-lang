...
describe('repeat while — ref + put', () => {
  ...
  it('counts down with ref and put', async () => {
    await expectBehavior({ script, receive: { id: '1', op: '@countdown', from: 'c' }, reply: { id: '1', 'bv-a': { x: 'Integer' }, re: { x: 0 }, to: 'c' } });
  });

  it('parens around condition with block body', async () => {
    await expectBehavior({ script, receive: { id: '2', op: '@parenCondition', from: 'c' }, reply: { id: '2', 'bv-a': { x: 'Integer' }, re: { x: 0 }, to: 'c' } });
  });

  it('single-line put form', async () => {
    await expectBehavior({ script, receive: { id: '3', op: '@singleLinePut', from: 'c' }, reply: { id: '3', 'bv-a': { x: 'Integer' }, re: { x: 0 }, to: 'c' } });
  });

  it('at end of function returns null (block never runs)', async () => {
    await expectBehavior({ script, receive: { id: '4', op: '@nullNeverRuns', from: 'c' }, reply: { id: '4', 'bv-a': { result: 'Integer | null' }, re: { result: null }, to: 'c' } });
  });
});
...
describe('repeat while — state mutation loop', () => {
  it('drains x to 0 and accumulates y to 10', async () => {
    const script = `
      ...
      -> x, y as Integer
    `;
    await expectBehavior({ script, receive: { id: '1', op: '@drain', from: 'c' }, reply: expect.objectContaining({ id: '1', re: [0, 10], to: 'c' }) });
  });
});
...
describe('repeat while — parenthesized condition (stateful)', () => {
  it('parens around condition with block body', async () => {
    const script = `
      ...
      -> x as Integer
    `;
    await expectBehavior({ script, receive: { id: '1', op: '@test', from: 'c' }, reply: expect.objectContaining({ id: '1', re: [0], to: 'c' }) });
  });
...
describe('repeat while — lexical scope', () => {
  it('reads and writes actor state inside block body', async () => {
    const script = `
      ...
      -> x as Integer
    `;
    await expectBehavior({ script, receive: { id: '1', op: [[3], '@test'], 'bv-a': [['Integer']], from: 'c' }, reply: expect.objectContaining({ id: '1', re: [9], to: 'c' }) });
  });
  ...
  it('reads and writes actor state inside single-line body', async () => {
    const script = `
      ...
      -> x as Integer
    `;
    await expectBehavior({ script, receive: { id: '1', op: [[5], '@test'], 'bv-a': [['Integer']], from: 'c' }, reply: expect.objectContaining({ id: '1', re: [5], to: 'c' }) });
  });
});
...
describe('repeat while — evaluates to null (stateful)', () => {
  it('at end of function returns null (block runs)', async () => {
    const script = `
      ...
      -> x, :result
    `;
    await expectBehavior({ script, receive: { id: '1', op: '@test', from: 'c' }, reply: expect.objectContaining({ id: '1', re: [0, { result: null }], to: 'c' }) });
  });
});
