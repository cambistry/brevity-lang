...
describe('...args rest binding', () => {
  ...
  it('named payload passes through — pack/splat roundtrip', async () => {
    await expectReply({ script, receive: { id: '1', op: [{ a: 1, b: 2 }, '@pass'], 'bv-a': [{ a: 'Integer', b: 'Integer' }], from: 'c' }, reply: { id: '1', 'bv-a': { a: 'Integer', b: 'Integer' }, re: { a: 1, b: 2 }, to: 'c' } });
  });

  it('positional payload passes through — pack/splat roundtrip', async () => {
    await expectReply({ script, receive: { id: '2', op: [[1, 2], '@pass'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '2', 'bv-a': ['Integer', 'Integer'], re: [1, 2], to: 'c' } });
  });
...
describe('Structure destructuring', () => {
  ...
  it('named — :a, :b = args', async () => {
    await expectReply({ script, receive: { id: '1', op: [{ a: 10, b: 20 }, '@namedTwo'], 'bv-a': [{ a: 'Integer', b: 'Integer' }], from: 'c' }, reply: { id: '1', re: { result: 10 }, to: 'c' } });
  });

  it('named — :a = args (extra keys ignored)', async () => {
    await expectReply({ script, receive: { id: '2', op: [{ a: 99, b: 2, c: 3 }, '@namedOne'], 'bv-a': [{ a: 'Integer', b: 'Integer', c: 'Integer' }], from: 'c' }, reply: { id: '2', re: { result: 99 }, to: 'c' } });
  });

  it('named — paren form', async () => {
    await expectReply({ script, receive: { id: '3', op: [{ a: 7, b: 8 }, '@namedParen'], 'bv-a': [{ a: 'Integer', b: 'Integer' }], from: 'c' }, reply: { id: '3', re: { result: 7 }, to: 'c' } });
  });

  it('positional — a, b = args', async () => {
    await expectReply({ script, receive: { id: '4', op: [[3, 4], '@posTwo'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '4', re: { result: 3 }, to: 'c' } });
  });

  it('positional — a = args[0]', async () => {
    await expectReply({ script, receive: { id: '5', op: [[42, 99, 1], '@posIndex'], 'bv-a': [['Integer', 'Integer', 'Integer']], from: 'c' }, reply: { id: '5', re: { result: 42 }, to: 'c' } });
  });

  it('positional — paren form', async () => {
    await expectReply({ script, receive: { id: '6', op: [[5, 6], '@posParen'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '6', re: { result: 5 }, to: 'c' } });
  });

  it('positional — trailing-comma paren form', async () => {
    await expectReply({ script, receive: { id: '7', op: [[11, 22], '@posTrailing'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '7', re: { result: 11 }, to: 'c' } });
  });

  it('mixed — a, b, :c = args', async () => {
    await expectReply({ script, receive: { id: '8', op: [[1, 2, { c: 3 }], '@mixedThree'], 'bv-a': [['Integer', 'Integer', { c: 'Integer' }]], from: 'c' }, reply: { id: '8', re: { result: 1 }, to: 'c' } });
  });

  it('mixed — paren form, uses named field', async () => {
    await expectReply({ script, receive: { id: '9', op: [[1, 2, { c: 99 }], '@mixedParen'], 'bv-a': [['Integer', 'Integer', { c: 'Integer' }]], from: 'c' }, reply: { id: '9', re: { result: 99 }, to: 'c' } });
  });

  it('key-mapped — a: x = args', async () => {
    await expectReply({ script, receive: { id: '10', op: [{ a: 55 }, '@keyMap'], 'bv-a': [{ a: 'Integer' }], from: 'c' }, reply: { id: '10', re: { result: 55 }, to: 'c' } });
  });

  it('key-mapped — paren form', async () => {
    await expectReply({ script, receive: { id: '11', op: [{ a: 77 }, '@keyMapParen'], 'bv-a': [{ a: 'Integer' }], from: 'c' }, reply: { id: '11', re: { result: 77 }, to: 'c' } });
  });
});
...
describe('Structure accessors', () => {
  ...
  it('args[0] reads first positional', async () => {
    await expectReply({ script, receive: { id: '1', op: [[42, 99], '@accessFirst'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '1', re: { result: 42 }, to: 'c' } });
  });

  it('args[1] reads second positional', async () => {
    await expectReply({ script, receive: { id: '2', op: [[10, 20], '@accessSecond'], 'bv-a': [['Integer', 'Integer']], from: 'c' }, reply: { id: '2', re: { result: 20 }, to: 'c' } });
  });

  it('args["a"] reads named field', async () => {
    await expectReply({ script, receive: { id: '3', op: [{ a: 'hello' }, '@accessNamed'], 'bv-a': [{ a: 'Text' }], from: 'c' }, reply: { id: '3', re: { result: 'hello' }, to: 'c' } });
  });
});
...
describe('Structure constructor', () => {
  ...
  it('Structure-stored function observes capture-time binding', async () => {
    await expectReply({ script, receive: { id: '3', op: '@constructLive', from: 'c' }, reply: { id: '3', 'bv-a': ['Integer'], re: [10], to: 'c' } });
  });
...
describe('Structure return — bv-a coercion', () => {
  ...
  it.skip('single-arity Structure returns [[100]] with bv-a', async () => {
    await expectReply({ script, receive: { id: '1', op: '@rawStructureOneArity', from: 'c' }, reply: { id: '1', 'bv-a': ['Structure'], re: [[100]], to: 'c' } });
  });

  it.skip('two-arity Structure returns [[100, 200]] with bv-a', async () => {
    await expectReply({ script, receive: { id: '2', op: '@rawStructureTwoArity', from: 'c' }, reply: { id: '2', 'bv-a': ['Structure'], re: [[100, 200]], to: 'c' } });
  });
});
