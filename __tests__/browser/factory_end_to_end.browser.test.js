...
//            `<<@0>>` → `<<factory.bv @0>>` on the wire to HTML.
//   Phase 4: HTML walks children, decomposes `<<factory.bv @0>>` →
//            `<<factory.bv>> @0`, posts subscribe to factory.bv. The
//            factory's @0 closure replies with current value; HTML
//            updates text node.
//   Plus: set@content mutation on Factory replays the @0 closure,
//         which pushes a new `re` to HTML, which updates the text node.
//
...
describe('factory end-to-end', () => {
  ...
  it('Factory.create returns element address whose text reflects @content', async () => {
    ...
    // Invoke Factory's @create. Factory emits a `new` to HTML with
    // children=[<<@0>>]; runtime prepends factory.bv → <<factory.bv @0>>;
    // HTML creates the element, subscribes to <<factory.bv>> @0, receives
  ...
  it('set@content on Factory propagates to the HTML element text', async () => {
    const page = await loadPage(html, { sources: { '/factory.bv': factorySource } });
    const factory = await page.connectActor('factory.bv');
